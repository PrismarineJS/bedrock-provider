...
export function getVersion (version: int | string): { str: string, int: int } {
  if (typeof version === 'number') {
    ...
    // @ts-expect-error : these are strings but get turned to numbers when shifting :)
...
export class BlockFactory {
  ...
  initialize (bedrockVersion: int | string) {
    ...
    for (let i = 0; i < maxStateId; i++) {
      const block = this.Block.fromStateId(i, 0)
      const props = block.getProperties()
      const bss = BlockFactory.buildBSS('minecraft:' + block.name, props)
      d.jss2jsid[bss] = i
  ...
  static buildBSS (name, states) {
    ...
    const bss = name + '[' + (str.endsWith(',') ? str.slice(0, -1) : str) + ']'
    return bss
  }
  ...
  getSimilarRuntimeID (name: string, version?) {
    const bsses = this.get(version).bss2brid
  ...
  getRuntimeID (name: string, states, version?): int {
    const bss = BlockFactory.buildBSS(name, states)
    if (!this.get(version)) {
      // console.log('init',name,states,version)
      this.initialize(version)
    }
    const a1 = this.get(version).bss2brid[bss]
    if (!a1) {
...
//     const jsid = blockFactory.getJSIDFromBRID(i)
